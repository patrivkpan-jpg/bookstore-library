...
var Main = {
   ...
   * @param {*} element 
   */
  openBookModal: function openBookModal(element) {
    var showBookModal = $('#showBookModal');
    var title = element.find('h5').text();
    var author = element.find('p').text();
    var img_src = element.find('img').attr('src');
    $('#showBookModalTitle').text(title);
    $('#showBookModalAuthor').text("by ".concat(author));
   ...
   * @returns 
   */
  getPopulateElement: function getPopulateElement(book) {
    return "\n            <div class=\"col-md-4 book-item\" data-id=\"".concat(book.id, "\">\n                <div class=\"card mb-4 shadow-sm\">\n                    <img src=\"").concat(book.cover_img, "\" alt=\"book-cover\">\n                    <div class=\"card-body\">\n                        <h5 class=\"card-title\">").concat(book.title, "</h5>\n                        <p class=\"card-text\">").concat(book.author, "</p>\n                        <a class=\"btn-edit-book btn btn-primary\" data-bs-toggle=\"modal\" data-bs-target=\"#updateBookModal\" data-id=\"").concat(book.id, "\">Edit</a>\n                        <a class=\"btn-delete-book btn btn-danger\" data-id=\"").concat(book.id, "\">Delete</a>\n                    </div>\n                </div>\n            </div>\n        ");
  },
  /**
   * Fill out bookstore with populated element
   */
  populateBookstore: function populateBookstore() {
    var bookstore = $('#bookstore');
    Main.books.forEach(function (book) {
      var populateElement = Main.getPopulateElement(book);
   ...
   * @returns 
   */
  prepareFormData: function prepareFormData(method) {
    var bookTitleInputSelector = $("#".concat(method, "BookTitleInput"));
    var bookAuthorInputSelector = $("#".concat(method, "BookAuthorInput"));
    var bookCoverInputSelector = $("#".concat(method, "BookCoverInput"));
    var bookTitleValue = bookTitleInputSelector.val();
    var bookAuthorValue = bookAuthorInputSelector.val();
    var bookCoverValue = bookCoverInputSelector[0].files[0];
   ...
   * @param {*} method 
   */
  concludeDataChange: function concludeDataChange(method) {
    Main.getBooks();
    $("#".concat(method, "BookCloseBtn")).click();
    $("#".concat(method, "BookTitleInput")).val('');
    $("#".concat(method, "BookAuthorInput")).val('');
    $("#".concat(method, "BookCoverInput")).val('');
  },
  /**
   * Inset book
   */
   ...
   * @param {*} element 
   */
  showUpdateBook: function showUpdateBook(element) {
    var title = element.siblings('h5').first().text();
    var author = element.siblings('p').first().text();
  ...
  updateBook: function updateBook(id) {
    var data = Main.prepareFormData('update');
    $.ajax({
      method: 'POST',
      url: "".concat(this.baseUrl, "/").concat(id, "/?_method=PUT"),
      data: data,
      processData: false,
      contentType: false
    }).done(function (res) {
  ...
  deleteBook: function deleteBook(id) {
    if (confirm('Are you sure you want to delete this book?')) {
      $.ajax({
        method: 'POST',
        url: "".concat(this.baseUrl, "/").concat(id, "/?_method=DELETE")
      }).done(function (res) {
        $(".book-item[data-id=\"".concat(id, "\"]")).remove();
      }).fail(function (res) {
        alert('Something went wrong with the request.');
      });
    }
